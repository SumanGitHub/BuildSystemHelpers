...
if ($r.Success)
{
  ...
  $currentBuildNumber = $args[0];
  if($value -and $currentBuildNumber.EndsWith(0))
  {
    $currentBuildNumber = $currentBuildNumber.Substring(0, $currentBuildNumber.Length-1) + $value;
  }
  Write-Host $currentBuildNumber
  Update-AllAssemblyInfoFiles $currentBuildNumber;
}
else
{
  echo " ";
